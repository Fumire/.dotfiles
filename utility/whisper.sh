...
for f in "$@"; do
    ...
    if [[ "$f" == *.mp4 ]]; then
        ...
        whisper-cli -m "/Users/fumire/Library/CloudStorage/Dropbox/31_AI/whisper-model/ggml-large-v3.bin" --output-srt --language "${LANG:-ko}" --threads 8 --processors 8 --print-colors --print-confidence --file "${f/%.m4a/.mp3}"
        ...
        whisper-cli -m "/Users/fumire/Library/CloudStorage/Dropbox/31_AI/whisper-model/ggml-large-v3.bin" --output-srt --language "${LANG:-ko}" --threads 8 --processors 8 --print-colors --print-confidence --file "$f"
